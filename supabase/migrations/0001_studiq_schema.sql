...
-- Policies: users manage only their own row
...
-- Note: Supabase doesn't support IF NOT EXISTS on triggers directly; use a guard DO block
do $$
begin
  if not exists (
...
-- Helper: quick ownership check via join to users(auth_id)
-- used in policies below
-- no object needed; just reuse EXISTS subqueries in policies
...
create policy if not exists "materials_select_own"
  on public.course_materials for select to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = course_materials.user_id and u.auth_id = auth.uid()
    )
  );

create policy if not exists "materials_insert_own"
...
create policy if not exists "materials_update_own"
...
create policy if not exists "materials_delete_own"
...
create policy if not exists "finance_select_own"
  on public.finance_plans for select to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = finance_plans.user_id and u.auth_id = auth.uid()
    )
  );

create policy if not exists "finance_insert_own"
  on public.finance_plans for insert to authenticated
  with check (
    exists (
      select 1 from public.users u
      where u.id = user_id and u.auth_id = auth.uid()
...
create policy if not exists "finance_update_own"
  on public.finance_plans for update to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = finance_plans.user_id and u.auth_id = auth.uid()
...
create policy if not exists "finance_delete_own"
  on public.finance_plans for delete to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = finance_plans.user_id and u.auth_id = auth.uid()
...
create policy if not exists "wallets_select_own"
  ...
  using (
    exists (
      select 1 from public.users u
      where u.id = wallets.user_id and u.auth_id = auth.uid()
    )
  );
...
create policy if not exists "tx_select_own"
  ...
  using (
    exists (
      select 1 from public.users u
      where u.id = transactions.user_id and u.auth_id = auth.uid()
    )
  );

create policy if not exists "tx_insert_own"
...
create policy if not exists "tx_update_own"
...
create policy if not exists "tx_delete_own"
...
create policy if not exists "nfts_select_own"
  on public.nfts for select to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = nfts.user_id and u.auth_id = auth.uid()
    )
  );

create policy if not exists "nfts_insert_own"
  on public.nfts for insert to authenticated
  with check (
    exists (
      select 1 from public.users u
      where u.id = user_id and u.auth_id = auth.uid()
...
create policy if not exists "nfts_update_own"
  on public.nfts for update to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = nfts.user_id and u.auth_id = auth.uid()
...
create policy if not exists "nfts_delete_own"
  on public.nfts for delete to authenticated
  using (
    exists (
      select 1 from public.users u
      where u.id = nfts.user_id and u.auth_id = auth.uid()
